...
import UIKit
protocol ActivityTaskCompleted {
    func isActiveTaskCompleted(activeTask:Bool)
}

class ActivityTaskVC: LoadingView {
    
    @IBOutlet weak var addNewActivityLab: UILabel!
    @IBOutlet weak var imageContainerVIew: UIView!
    @IBOutlet weak var childIMage: UIImageView!
    
    @IBOutlet weak var activeNameLab: UILabel!
    @IBOutlet weak var activeNameTF: UITextField!
    @IBOutlet weak var activeNameValidationLab: UILabel!
    ...
    var childsIDs = ""
    var selectAll = false
    var activeTaskDelegate : ActivityTaskCompleted?
    ...
    func setUpLocalizeElement(){
        addNewActivityLab.setTitle(For: "add New Activity")
        activeNameLab.setTitle(For: "Activity name")
        activeNameTF.setTextfieldPlaceolder(Placeholder: "Enter the activity name")
...
//        }
        ...
        activeNameValidationLab.isHidden = false
        numberOfPointsValidationLab.isHidden = false
        selectChildValidationLab.isHidden = false
        
        activeNameValidationLab.text = "Please Enter the activity name".localized
        ...
        guard !activeNameTF.text!.isEmpty else {
//            HelperTools.ShowErrorMassge(massge: "Please Enter the activity name", title: .error)
            return false
        }
        activeNameValidationLab.isHidden = true
...
//                HelperTools.ShowIntervalMassge(massge: nil, key: response.message ?? "")
                DispatchQueue.main.asyncAfter(deadline: .now() + 1) {
                    self.activeTaskDelegate?.isActiveTaskCompleted(activeTask: true)
                    self.dismiss(animated: true, completion: nil)
                }
            }else{
...
extension ActivityTaskVC :UINavigationControllerDelegate,UIImagePickerControllerDelegate{
...
extension ActivityTaskVC : UITableViewDelegate ,UITableViewDataSource {
